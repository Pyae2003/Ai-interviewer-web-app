...
import { updateCategoryGroup } from "../actions/update-category-groups";
import { UpdateCategoryGroupsInput, updateCategoryGroupsSchema } from "../schema/update-category-groups.schema";

export function UpdateCategoryGroupsForm({data} : GetCategoryGroupResponse) {
  ...
  const { execute, status } = useAction(updateCategoryGroup, {
    ...
    onError: ({ error }) => {
      toast.error(
        error.serverError?.message ?? "Failed to update category group",
      );
    },
  });

  const form = useForm<UpdateCategoryGroupsInput>({
    resolver: zodResolver(updateCategoryGroupsSchema),
    defaultValues: {
      id : data.id,
      name: data.name,
      slug: data.slug,
  ...
  const onSubmit = (data: UpdateCategoryGroupsInput) => {
    if (isLoading) return;
